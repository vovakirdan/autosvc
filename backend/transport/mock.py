...
from backend.uds.dtc import encode_dtc, status_to_byte
...
class MockTransport(CanTransport):
    ...
    def _ecu_from_request_id(self, can_id: int) -> Optional[int]:
        if 0x7E0 <= can_id <= 0x7EF:
            return can_id - 0x7E0
        return None

    def _build_response(self, ecu: int, data: bytes) -> Optional[bytes]:
        service = data[0]
        if service == 0x10:
